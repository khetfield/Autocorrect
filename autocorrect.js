 ...
 */
const getRelevantSentences = (criterion, sentences) => {return sentences.filter(sentence => criterion(sentence));};
 ...
 */
const getDistanceByLength = (str1, str2) => {return Math.abs(str1.length - str2.length);} 
...
const getClosestWord = (word, words, distanceFn, threshold) => {
  ...
  for (const candidate of words) {const distance = distanceFn(word, candidate);
      if (distance < minDistance) {minDistance = distance; closestWord = candidate;}}
...
const getClosestWordByLength = (word, words, threshold) => {
  let closestWord = word;
  let minDistance = Math.abs(word.length - closestWord.length);
  
  for (const w of words) {const distance = Math.abs(word.length - w.length);
